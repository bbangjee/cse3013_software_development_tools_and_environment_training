...
void ofApp::draw() {
	...
	if(bShowInfo) {
		...
		myFont.drawString("[O] Open File  [D] Toggle DFS  [B] Toggle BFS [F] Fullscreen", 15, ofGetHeight()-40);
	}
...
bool ofApp::BFS() {
	// queue 초기화
	visited_bfs.assign(HEIGHT, vector<bool>(WIDTH, false));
	parent_bfs.assign(HEIGHT, vector<pair<int, int>>(WIDTH, {-1, -1}));
	visitOrder_bfs.clear();
	...
	q.push(startpoint);
	visited_bfs[startpoint.first][startpoint.second] = true;
	visitOrder_bfs.push_back(startpoint);
	...
	while(!q.empty()) {
		...
		for (auto &neighbor : graph[curr.first][curr.second]) {
			...
			if (visited_bfs[ny][nx] == false) {
				...
				parent_bfs[ny][nx] = curr;
				visitOrder_bfs.push_back({ny, nx});
				q.push({ny, nx});
			}
		}
	}
	...
	if (found == true) {
		pair<int, int> temp = endpoint;
		while (temp != startpoint) {
			finalPath_bfs.push_back(temp);
			int temp_y = temp.first;
			int temp_x = temp.second;
			temp = parent_bfs[temp.first][temp.second];
		}
		finalPath_bfs.push_back(startpoint);
		cout << "BFS Success\n";
		return true;
	}
	cout << "BFS Fail\n";
...
void ofApp::bfsdraw() {
	...
	ofSetColor(200, 200, 200);
	for (int i = 1; i < visitOrder_bfs.size(); i++) {
		pair<int, int> temp = visitOrder_bfs[i];
		int curr_y = temp.first;
		int curr_x = temp.second;
		pair<int, int> temp_parent = parent_bfs[curr_y][curr_x];
		int parent_y = temp_parent.first;
		int parent_x = temp_parent.second;
	...
	ofSetColor(0, 0, 255);
	for (int i = 0; i < finalPath_bfs.size() - 1; i++) {
		pair<int, int> temp = finalPath_bfs[i];
		int curr_y = temp.first;
		int curr_x = temp.second;
		pair<int, int> temp_parent = finalPath_bfs[i + 1];
		int parent_y = temp_parent.first;
		int parent_x = temp_parent.second;
