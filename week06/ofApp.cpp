...
void ofApp::keyReleased(int key){
	...
	if (key == OF_KEY_RIGHT){
		...
		cout << "Selcted Dot Coordinate is (" << dot[selected_dot_index].x1 << ", " << dot[selected_dot_index].y1 << ")" << endl;
	}
	if (key == OF_KEY_LEFT){
	...
	for (ofBuffer::Line it = buffer.getLines().begin(), end = buffer.getLines().end(); it != end; ++it) {
		...
		if(words.size() == 1){
			...
			if(input_type == 0){
				...
				if(x1 < 0 || x1 > ofGetWidth() || x2 < 0 || x2 > ofGetWidth() || y1 < 0 || y1 > ofGetHeight() || y2 < 0 || y2 > ofGetHeight()) {
