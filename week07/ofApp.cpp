...
void ofApp::draw() {
  ...
  ofDrawRectangle(0, 0, 1024,
                  40);  // Top left corner at (50, 50), 100 wide x 100 high
  ofDrawRectangle(0, 728, 1024,
                  40);  // Top left corner at (50, 50), 100 wide x 100 high
  ofSetLineWidth(5);

  ofSetLineWidth(5);
  if (draw_flag) {
    /* COMSIL1-TODO 3 : Draw the line segment and dot in which water starts to
     flow in the screen. Note that after drawing line segment and dot, you have
     to make selected water start dot colored in red.
     */
...
void ofApp::keyReleased(int key) {
  if (key == 'l') {
    ...
    ofFileDialogResult openFileResult =
        ofSystemLoadDialog("Select a only txt for Waterfall");
  ...
  if (key == OF_KEY_RIGHT) {
    selected_dot_index = (selected_dot_index + 1) % num_of_dot;
    cout << "Selcted Dot Coordinate is (" << dot[selected_dot_index].x1 << ", "
         << dot[selected_dot_index].y1 << ")" << endl;
  }
  if (key == OF_KEY_LEFT) {
    selected_dot_index = (selected_dot_index - 1 + num_of_dot) % num_of_dot;
    cout << "Selcted Dot Coordinate is (" << dot[selected_dot_index].x1 << ", "
         << dot[selected_dot_index].y1 << ")" << endl;
  }
}
